...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { name: "Work", href: "/work"},
    { name: "Blog", href: "/blog" , ClassNames:"mr-5"},
    {
      name: "Hire Me",
      href: "/hireme",
    },
  ...
  return (
    <nav className="fixed font-[var(--font-outfit)] top-0 left-0 w-full z-50 transition-colors">
      <div className="container mx-auto flex items-center justify-between sm:p-5 p-6.5 md:justify-around">
        ...
        <motion.ul
          className="hidden md:flex items-center gap-0.5 text-[18px] backdrop-blur-sm bg-[#232223]/20 border border-[#ffffff10] rounded-lg px-1 py-1"
          initial="hidden"
          animate="show"
          variants={{
          ...
          {navLinks.map((link) => {
            ...
            return (
              ...
              >
                <Link
                  href={link.href}
                  className={`group flex items-center text-[12px] gap-0.5 px-3 py-1 rounded-lg transition-all duration-300 ${
                    link.name === "Book a Call"
                      ? "text-[#ffffffbd] dark:text-black ml-2 backdrop-blur-[20px] bg-white/10 hover:bg-white/20 dark:border-black/30 shadow-[0_0px_10px_1px_rgba(255,255,255,0.3)] dark:shadow-[0_0px_10px_0.9px_#FFFFFF66]"
                      : isActive
                      ? activeClass
                      : "text-[#ffffffd5] dark:text-black hover:bg-white/10 dark:hover:bg-white/10"
                  }`}
                >
                  {/* {link.icon && (
