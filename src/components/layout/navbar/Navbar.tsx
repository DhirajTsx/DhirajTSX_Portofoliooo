...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { name: "Hire Me", href: "/hireme", icon: <ConnectWithoutContactIcon fontSize="small" /> },
  ];
  ...
  {navLinks.map((link) => {
    ...
    return (
      <motion.li key={link.name} className="relative flex items-center" layout>
        <Link
          href={link.href}
         className="group flex items-center gap-1 px-3 py-2 rounded-full transition-all duration-300 hover:bg-black/10 dark:hover:bg-white/10"

        >
          <span
            className={`flex items-center justify-center ${
              isActive ? activeColors[link.name as ActiveColorKeys] : "text-white dark:text-black"
            }`}
          >
            {link.icon}
          </span>
